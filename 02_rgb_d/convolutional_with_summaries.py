...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    with tf.name_scope('inputs'):
        train_data_node = tf.placeholder(
            data_type(),
            shape=(BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS), name='train_data')
    ...
    # Global variables: parameters for the Lenet-5 network
    # conv1: 5*5 filter with depth 32
    with tf.name_scope('conv1'):
        W_conv1 = weight_variable([5, 5, NUM_CHANNELS, 32], name='W_conv1')
    ...
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([8*8*64, 512], name='W_fc1')
        b_fc1 = bias_variable([512], name='b_fc1')

    # fc2: 512 num_units with depth: NUM_LABELS
    with tf.name_scope('fc2'):
        W_fc2 = weight_variable([512, NUM_LABELS], name='W_fc2')
        b_fc2 = bias_variable([NUM_LABELS], name='b_fc2')
    ...
    def model(data, train=False):
        ...
        # dropout, create a placeholder for the kept probability
    ...
    with tf.name_scope('loss'):
        with tf.name_scope('loss_pre_reg'):
    ...
    #tf.scalar_summary('train_prediction', train_prediction)

    # Predictions for the test and validation, which we'll compute less often.
    #with tf.name_scope('eval_prediction'):
    eval_prediction = tf.nn.softmax(model(eval_data))
    #tf.scalar_summary('eval_prediction', eval_prediction)
    ...
    def eval_in_batches(data, sess):
        ...
        for begin in xrange(0, size, EVAL_BATCH_SIZE):
            ...
            if end <= size:
                ...
                # run the last 'EVAL_BATCH_SIZE' data
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.train.SummaryWriter('./board' + '/train', sess.graph)
        val_writer = tf.train.SummaryWriter('./board' + '/val', sess.graph)
